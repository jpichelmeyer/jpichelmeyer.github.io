...
class Archivist():
    ...
    @prepend_name
    def backup_helpers(self, for_real=False):
        
        abs_path_helpers = os.path.join(self.abs_path_project, 'helpers')
        helper_files = os.listdir(abs_path_helpers)
        for filename in helper_files:
            print(filename)
            if os.path.isfile(filename):
                base, ext = filename.split('.')
                
                abs_path_source = os.path.join(abs_path_helpers, filename) 
                abs_path_destination = ''
                
                # Accumulate destination path
                for extend in [
                    self.abs_path_project,
                    'helpers',
                    'backup',
                    base + f'_{self._get_date_time()}.' + ext,
                    ]:
                    abs_path_destination = os.path.join(abs_path_destination, extend)
                print(f'abs_path_destination:{abs_path_destination}')
                if for_real:
                    shutil.copy2(abs_path_source, abs_path_destination)
    ...
    # EXPANSION SLOT (Archivist): Compactify code (minimum number of lines
    # EXPANSION SLOT (Archivist): Expand code (maximum readability)
...
if __name__=='__main__':
    ...
    #
    #    live mod  : minimal
    #    file gen  : brand new folder, brand new copies
    #
    #R.update_headers()
    #R.archive(for_real=False)
    #R.archive(for_real=True)
    R.backup_helpers(for_real=True)
