...
def collect_item_specs(categories):
    ...
    Brightspace's convention (confirmed against a real grade export) is
    that an item's Weight is relative to the OTHER ITEMS IN ITS OWN
    CATEGORY, and should sum to 100 across that category -- the category's
    own pull on the final grade is handled separately by CategoryWeight.
    So item_weight here is 100 / num_items, evenly splitting 100% across
    the items, not the category's raw percentage."""
    print("\nFor each category, enter how many graded items it should have,")
    print("and how many points each item is worth. Each item's weight")
    print("within the category will be split evenly (summing to 100%).\n")
    ...
    for cat in categories:
        ...
        cat['item_weight'] = 100 / cat['num_items']
        print()
