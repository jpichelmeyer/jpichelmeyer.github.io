...
class Archivist():
    ...
    def update_header_with_path(self, file_path, ext):
        # 1. Get the current front content
        old_fc = self.scan_for_fc(file_path, ext)
        
        # 2. Calculate the relative path from the project root
        rel_path = os.path.relpath(file_path, self.abs_path_project)
        
        # 3. Create the comment based on extension
        templates = {
            "js": f"// {rel_path}",
            "css": f"/* {rel_path} */",
            "html": f"<!-- {rel_path} -->"
        }
        new_header = templates.get(ext, f"# {rel_path}") + "\n"

        # 4. Read the whole file and swap the header
        with open(file_path, 'r', encoding='utf-8') as f:
            full_content = f.read()
        
        # Replace only the first occurrence (the header)
        updated_content = full_content.replace(old_fc, new_header, 1)

        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(updated_content)
        print(f"Updated header for: {rel_path}")

    @prepend_name
    def restore_from_archive(self, label: str):
        # Construct the path to the specific backup
        archive_dir = os.path.join(self.abs_path_project, self.MAIN_STORE, label)
        
        if not os.path.exists(archive_dir):
            print(f"FAILED: Archive label '{label}' not found at {archive_dir}")
            return

        print(f"Restoring files from {label}...")
        
        # We only want these specific types for the restore
        restore_exts = ["html", "css", "js"]
        count = 0

        for filename in os.listdir(archive_dir):
            ext = filename.split('.')[-1].lower()
            if ext in restore_exts:
                src_file = os.path.join(archive_dir, filename)
                # This assumes the files go back to the project root
                # If they were in subfolders, you'd need a mapping file!
                dst_file = os.path.join(self.abs_path_project, filename)
                
                shutil.copy2(src_file, dst_file)
                print(f"  -> Restored: {filename}")
                count += 1
        
        print(f"Done! {count} files restored to project root.")
