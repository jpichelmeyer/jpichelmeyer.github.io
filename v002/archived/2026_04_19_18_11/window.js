...
export function maximizeWindow(appId) {
    ...
    if (!el.classList.contains('maximized')) {
        ...
        el.style.cssText = 'position:fixed; inset:0; width:100vw; height:100vh; display:grid; grid-template-rows:36px 1fr; overflow:hidden; z-index:500; border:none; box-shadow:none;';
    } else {
...
function createWindowElement(app, x, y) {
    ...
    el.style.cssText = `left:${x}px; top:${y}px; width:${app.width || 600}px; height:${app.height || 400}px; display:grid; grid-template-rows:36px 1fr; overflow:hidden;`;

    /*
        Layout (Warm Brutalist — titlebar INSIDE window):

        ┌─────────────────────────────────────────────────────┐
        │  APP LABEL ············ [↗]  [_]  [□]  [×]  ← titlebar
        ├─────────────────────────────────────────────────────┤
        │                                                     │
        │              .pos-window-body                       │
        │                                                     │
    ...
    */
    ...
    titlebar.className = 'pos-titlebar';
    titlebar.dataset.dragTarget = '';

    const label = document.createElement('div');
    label.className = 'pos-win-label';
    label.textContent = (app.label || app.id).toUpperCase();

    const strip = document.createElement('div');
    strip.className = 'pos-ctl-strip';

    const btnPop   = makeCtl('↗', 'pos-ctl-pop',   'Pop out');
    const btnMin   = makeCtl('−', 'pos-ctl-min',   'Minimize');
    ...
    el.append(titlebar, body, resizeHandle);
