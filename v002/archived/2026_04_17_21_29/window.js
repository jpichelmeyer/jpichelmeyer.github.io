/*====/*=====================================================================
    v004/pos/gui/window.js

    Changes from v002:
    ─────────────────
    • NO sidebar tab — windows have zero chrome
    • Control buttons (.pos-ctl) float ABOVE the window as flat
      filled squares (Diagon style): [LABEL] [↗ pop] [_ min] [□ max] [× close]
    • Pop-out button opens a detached browser window at a fixed size
    • Mouse-wheel on a focused window (or desktop) scales it via CSS
      transform — wheel event is captured (preventDefault) so the
      browser never scrolls or zooms
    • Keyboard shortcut pass-through: while fullscreen, the OS owns
      all input (we call e.preventDefault() on wheel inside the
      fullscreen element)
=====================================================================*/
