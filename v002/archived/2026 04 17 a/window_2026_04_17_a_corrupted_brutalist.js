...
export function maximizeWindow(appId) {
    ...
    if (!el.classList.contains('maximized')) {
        // Save current state before going big
        el.dataset.preMaxStyle = el.style.cssText;
        el.classList.add('maximized');
        // Force clean alignment to top-left
        el.style.top = "0";
        el.style.left = "0";
...
function createWindowElement(app, x, y) {
    ...
    el.innerHTML = `
        <div class="pos-titlebar" data-drag-target>
            <div class="pos-traffic-lights">
                <button class="pos-tl pos-tl-close">×</button>
                <button class="pos-tl pos-tl-min">_</button>
                <button class="pos-tl pos-tl-max">□</button>
            </div>
            <div class="pos-titlebar-label">${app.label}</div>
        </div>
    ...
    // Event Listeners
    el.querySelector('.pos-tl-close').onclick = () => closeWindow(app.id);
    el.querySelector('.pos-tl-max').onclick = () => maximizeWindow(app.id);
