...
`,
    ...
    saveState: () => {
        const editor = document.querySelector('.tp-editor-textarea');
        return { 
            code: editor?.value || "", 
...
function launchTreepiler(body, appId) {
    ...
    // Listener ::: Tab key → insert spaces
    ...
    // Listener ::: Mouse wheel -> font size +/- in editor
    let fontSize = 12;
    textarea.addEventListener('wheel', e=> {
    	...
    	// Line height and line number height stay proportional
    	const lh = fontSize * 1.6;
    	textarea.style.lineHeight = `${lh}px`;
    	overlay.style.lineHeight  = `${lh}px`;
    	lineNums.style.lineHeight = `${lh}px`;
    	// Update the fixed-height spans so they stay in sync
